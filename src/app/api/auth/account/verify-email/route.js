...
export async function POST(request) {
    ...
    if (!eMail || code === null) {
        return NextResponse.json(
            new ResponseModel(400, 'Missing required fields: eMail, code'),
            { status: 400 }
        );
    }
    ...
    try {
        const upstreamResponse = await fetch(`https://${host}/Agent/Account/VerifyEMail`, {
            method: 'POST',
            headers: {
        ...
        return NextResponse.json(
            new ResponseModel(
                ...
                upstreamResponse.ok ? 'E-mail verified' : 'E-mail verification failed',
                responseData
            ),
