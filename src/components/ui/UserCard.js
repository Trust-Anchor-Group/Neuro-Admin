import { BsThreeDots } from "react-icons/bs";

const UserCard = ({ type }) => {
  return (
    <div className="rounded-2xl bg-[var(--brand-navbar)] shadow-lg p-6 flex-1 min-w-[200px] transition-transform transform hover:scale-105">
      <div className="flex justify-between items-center mb-4">
        <span className="text-[10px] bg-green-100 text-green-600 px-3 py-1 rounded-full font-semibold uppercase tracking-wide">
          This Month {type}
        </span>
        {/* More Icon */}
        <BsThreeDots className="text-[var(--brand-text-secondary)] text-xl cursor-pointer hover:text-[var(--brand-text)]" />
      </div>
      <h1 className="text-3xl font-bold text-[var(--brand-text)] mb-2">1,234</h1>
      <h2 className="text-md font-medium text-[var(--brand-text-secondary)] capitalize">{type}</h2>
    </div>
  );
};

export default UserCard;