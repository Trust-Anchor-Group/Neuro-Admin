...
export const InputField = ({labelText,name,profil}) => {
    ...
    return (
      <div>
        ...
        <p className='text-text16 max-sm:text-md py-3 pl-4 border
         rounded-lg border-[var(--brand-border)] w-full bg-[var(--brand-input-background)] text-[var(--brand-text-primary)] whitespace-pre-line'>{name}</p>
      </div>
    )
  }
  ...
  return (
    <div className='grid grid-cols-3 items-center justify-center border-b-2 border-[var(--brand-border)] animate-fade-in'>
        ...
        <p className='text-text16 max-sm:text-md my-2 whitespace-pre-line'>{name}</p>
    </div>
  )
}
